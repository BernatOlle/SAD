...
class Game:
        ...
        if self.turn[player] == True:
            ...
            if pos != -1:
                ...
                if (self.piece_sel == self.constants.TORRE_1 or self.piece_sel == self.constants.TORRE_2):
                    self.moveCorrect = self.verificarTorre(
                        x_vella, y_vella, int(x[1]), int(y[1]), player)
                    ...
                    self.moveCorrect = self.verificarPeo(
                        x_vella, y_vella, int(x[1]), int(y[1]), player)
                    ...
                    self.moveCorrect = self.verificarAlfil(
                        x_vella, y_vella, int(x[1]), int(y[1]), player)
                    
                elif self.piece_sel == self.constants.REINA:
                    self.moveCorrect = self.verificarReina(
                        x_vella, y_vella, int(x[1]), int(y[1]), player)
                    
                elif self.piece_sel == self.constants.REI:
                    self.moveCorrect = self.verificarRei(
                        x_vella, y_vella, int(x[1]), int(y[1]), player)
    ...
    def verificarTorre(self, x_vella, y_vella, x_nova, y_nova, player):
        ...
        if (x_vella != x_nova and y_vella == y_nova) or (x_nova == x_vella and y_nova != y_vella):
            ...
            Torre = self.check_straight(
                (x_vella, y_vella), (x_nova, y_nova), player)
    ...
    def verificarReina(self, x_vella, y_vella, x_nova, y_nova, player):
        ...
        print("CHECK REINA")
        mov_diag = False
        movHoritz = self.verificarTorre(
            x_vella, y_vella, x_nova, y_nova, player)
        mov_diag = self.verificarAlfil(
            x_vella, y_vella, x_nova, y_nova, player)
