...
def threaded_client(conn, player):
    ...
    conn.send(pickle.dumps(players[player])) # envia una copia serialitzada del player 
    reply = " "
    while True:
        try:
            data = pickle.loads(conn.recv(2048)) #recibe datos del cliente cuando se mueve
            players[player] = data 
